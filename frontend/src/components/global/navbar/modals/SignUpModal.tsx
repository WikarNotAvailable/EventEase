  ...
  return (
      <GlobalModal isOpen={isOpen} onClose={onClose} headerText="Sign up">
        ...
        <Flex justify="space-between" w="100%">
          <Input width="45%" heading="First name" type="text" onChange={(e: any) => dispatchForm({ type: 'FIRSTNAME_CHANGE', val: e.target.value })} />
          <Input width="45%" heading="Surname" type="text" onChange={(e: any) => dispatchForm({ type: 'SURNAME_CHANGE', val: e.target.value })} />
        </Flex>
        <Flex justify="space-between" w="100%">
          <Input width="45%" heading="Date of birth" type="date" onChange={(e: any) => dispatchForm({ type: 'BIRTH_DATE_CHANGE', val: e.target.value })} />
          <Input width="45%" heading="Phone number" type="tel" onChange={(e: any) => dispatchForm({ type: 'PHONE_NUMBER_CHANGE', val: e.target.value })} />
        </Flex>
        <Input width="100%" heading="Email" type="email" onChange={(e: any) => dispatchForm({ type: 'EMAIL_CHANGE', val: e.target.value })} />
        <Input width="100%" heading="Password" type="password" onChange={(e: any) => dispatchForm({ type: 'PASSWORD_CHANGE', val: e.target.value })} />
        <Input width="100%" heading="Repeat password" type="password" onChange={(e: any) => dispatchForm({ type: 'PASSWORD_CONFIRMATION_CHANGE', val: e.target.value })} />
